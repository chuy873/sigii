<?php
/* Pagina Administrar Modelos por proyecto
 Se despliega la lista de todos los modelos registrados en la base de datos de acuerdo al proyecto seleccionado.
...
*/
$pageTitle = "SIPROI | Administrar Modelos por proyecto";
...
$data ="SELECT p.nombre AS proyecto, m.nombre AS modelo, p.promotor, p.segmento, ".
		...
		"INNER JOIN modelo m " .
		"ON m.proyecto_idproyecto = p.idproyecto AND p.idproyecto ='".$idProyecto."'" ;
$cont = 0;
$dataModelo = mysql_query( $data );
$nombreProy="SELECT nombre, tipo FROM proyecto WHERE idproyecto='".$idProyecto."'";
$result=mysql_query($nombreProy);
...
<div class="row">
	<div class="span10 offset2">
		<form class="form-horizontal" action="#" method="post">
			<fieldset>
				<legend>
					<i class="icon-fullscreen"></i> Administrar modelos de
				...
				<table class="table">
					<tr>
						...
						<th style='text-align: left; width: 200px; padding: 5px;'>Metros
							cuadrados</th>
					...
					<tr>
						...
						<td><?php if($data["tipo"]=="horizontal"){?> <a
							href="editarModeloHorizontal.php?id=<?php echo $data["idmodelo"]?>"
							class="btn btn-primary"><i class="icon-edit icon-white"></i>
								Editar</a> <?php } else if($data["tipo"]=="vertical"){?> <a
							href="editarModeloVertical.php?id=<?php echo $data["idmodelo"]?>"
							class="btn btn-primary"><i class="icon-edit icon-white"></i>
								Editar</a> <?php }?>
						</td>
						<td><a href="#" class="openDeleteModelo btn btn-danger"
							data-id="<?php echo $data["idmodelo"]?>"><i
...
?>
				...
				<div class="form-actions">
					<?php if($data1["tipo"]=="horizontal"){?>
					<a href="registrarModeloHorizontal.php" class="btn btn-inverse"><i
						class="icon-plus-sign icon-white"></i> Agregar modelo
						horizontal...</a>
					<?php } 
					if($data1["tipo"]=="vertical"){
						?>
					<a href="registrarModeloVertical.php" class="btn btn-inverse"><i
						class="icon-plus-sign icon-white"></i> Agregar modelo vertical...</a>
					<?php }?>
					<a href="seleccionarProyAdmin.php" class="btn"><i
						class="icon-remove-circle"></i> Regresar</a>
				</div>
			</fieldset>
		</form>
	</div>
