	...
	<div class="tabbable tabs-left">
			...
			<li>
				<form action="control/exportarExcel.php" method="post"
					...
					<input type="hidden" id="datos_a_enviar" name="datos_a_enviar" /> <input
						type="hidden" name="tipo" value="resumen" />
				</form>
			</li>
		</ul>
				...
				<td>
					<div class="tab-content">
						...
						foreach($segmentos as $segmento){
							?>
						<div class="tab-pane <?php if($tab==1){?>active<?php }?>"
							id="tab<?php echo $tab?>">
							<div class="row-fluid">
		...
		while ($data = mysql_fetch_array($result, MYSQL_ASSOC)) {?>
										<tr>
											...
											<td><?php echo $data["nombreP"]?>
											</td>
											<td><?php if($data["tiempoMercado"]){
												echo round($data["unidadesVendidas"]/$data["tiempoMercado"],2);
											}?>
											</td>
											<td>$<?php echo number_format(round($data["precio"],2), 2, '.', ',')?>
											</td>
											<td><?php echo round($data["metrosCuadrados"],0)?></td>
											<td><?php echo round($data["metrosCuadradosTerr"],0)?></td>
											<td>$<?php if($data["metrosCuadrados"]!=0){echo number_format(round(($data["precio"]/$data["metrosCuadrados"]),2), 2, '.', ',');}?>
											</td>
											<td><?php echo $data["unidades"]?></td>
											<td><?php echo $data["unidadesVendidas"]?></td>
											<td><?php echo ($data["unidades"]-$data["unidadesVendidas"])?>
											</td>
